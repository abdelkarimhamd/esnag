import { Alert, Box, Button, Card, CardContent, FormControl, Grid, InputLabel, MenuItem, Paper, Select, Stack, Switch, Table, TableBody, TableCell, TableHead, TableRow, TextField, Typography, } from '@mui/material';
...
const toErrorMessage = (error, fallback) => normalizeApiError(error, fallback).message;
export const OpsAdminPage = () => {
    ...
    const loadFeatureFlags = useCallback(async () => {
        ...
        const response = await api.get('/api/ops/feature-flags', {
            params: {
        ...
        setFeatureRows(rows);
        setFeatureDraft(rows.reduce((acc, row) => {
            acc[row.feature_key] = row.resolved_enabled;
            return acc;
        }, {}));
    }, [canManageFlags, selectedProjectNumericId]);
    const loadUsageLimits = useCallback(async () => {
        if (!canManageLimits) {
            return;
        }
        const response = await api.get('/api/ops/usage-limits');
    ...
    const loadSecuritySettings = useCallback(async () => {
        ...
        const response = await api.get('/api/ops/security');
    ...
    const loadSupportData = useCallback(async () => {
        ...
        const [membersResponse, invitesResponse] = await Promise.all([
            ...
            api.get('/api/ops/support/invites', {
                params: { per_page: 20 },
            }),
        ]);
    ...
    useEffect(() => {
        if (!activeOrganizationId || !canManageFlags) {
            return;
        }
        void loadFeatureFlags();
    }, [activeOrganizationId, canManageFlags, loadFeatureFlags]);
    useEffect(() => {
        if (!activeOrganizationId || !canViewHealth) {
            return;
        }
        void loadHealth();
    }, [activeOrganizationId, canViewHealth, loadHealth]);
    ...
    const saveUsageLimits = async () => {
        ...
        setSuccess(null);
        try {
            await api.put('/api/ops/usage-limits', {
    ...
    const saveSecuritySettings = async () => {
        ...
        setSuccess(null);
        try {
            await api.put('/api/ops/security', {
    ...
    const heroMetrics = useMemo(() => ({
        flags: featureRows.length,
        queueDepth: health?.queue_depth.framework_jobs ?? 0,
        syncErrorRate: health?.sync.error_rate_percent ?? 0,
        invites: invites?.total ?? 0,
    }), [featureRows.length, health?.queue_depth.framework_jobs, health?.sync.error_rate_percent, invites?.total]);
    if (!canAccess) {
    ...
    return (<Stack spacing={2}>
      ...
      <PageHero title="Ops Console" description="Tenant controls for feature flags, quotas, health metrics, security policy, and support tools." actions={<Button variant="outlined" onClick={() => void loadAll()} disabled={busy} sx={{ borderColor: 'rgba(255,255,255,0.44)', color: '#FFFFFF' }}>
            Refresh
          </Button>}/>

      <Grid container spacing={1.2}>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard label="Flags" value={heroMetrics.flags} tone="primary"/>
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard label="Queue Depth" value={heroMetrics.queueDepth} tone="secondary"/>
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard label="Sync Error Rate %" value={heroMetrics.syncErrorRate} tone="warning"/>
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard label="Invites" value={heroMetrics.invites} tone="neutral"/>
        </Grid>
      </Grid>

      {canManageFlags && (<Paper sx={{ p: 2 }}>
          <Stack spacing={2}>
            <Box display="flex" justifyContent="space-between" alignItems="center" gap={2} flexWrap="wrap">
              ...
              <FormControl size="small" sx={{ minWidth: 280 }}>
                ...
                <Select labelId="ops-feature-project" label="Scope" value={selectedProjectId} onChange={(event) => setSelectedProjectId(event.target.value ? Number(event.target.value) : '')}>
            ...
            <Table size="small">
              ...
              <TableBody>
                {featureRows.map((row) => (<TableRow key={row.feature_key}>
                    <TableCell>
                    ...
                    <TableCell align="center">
                      <Switch checked={Boolean(featureDraft[row.feature_key])} onChange={(event) => setFeatureDraft((current) => ({
                    ...current,
      ...
      {canManageSecurity && securitySettings && (<Paper sx={{ p: 2 }}>
          <Stack spacing={2}>
            ...
            <Grid container spacing={1.5}>
              <Grid size={{ xs: 12, md: 3 }}>
                ...
                <Switch checked={securitySettings.mfa_required_web} onChange={(event) => setSecuritySettings((current) => current
                ? {
                    ...current,
              ...
              <Grid size={{ xs: 12, md: 3 }}>
                ...
                <Switch checked={securitySettings.mfa_required_mobile} onChange={(event) => setSecuritySettings((current) => current
                ? {
                    ...current,
              ...
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField size="small" label="Trusted device days" type="number" value={securitySettings.mobile_device_trust_days} onChange={(event) => setSecuritySettings((current) => current
                ? {
                    ...current,
              ...
              <Grid size={{ xs: 12, md: 3 }}>
                ...
                <Switch checked={securitySettings.enforce_ip_allowlist} onChange={(event) => setSecuritySettings((current) => current
                ? {
                    ...current,
            ...
            <TextField size="small" label="IP allowlist (newline or comma separated; CIDR supported)" value={ipAllowlistDraft} onChange={(event) => setIpAllowlistDraft(event.target.value)} multiline minRows={3}/>

            <Grid container spacing={1.5}>
              <Grid size={{ xs: 12, md: 6 }}>
                <FormControl size="small" fullWidth>
                  <InputLabel id="ops-antivirus-mode">Antivirus Mode</InputLabel>
                  <Select labelId="ops-antivirus-mode" label="Antivirus Mode" value={securitySettings.antivirus_mode} onChange={(event) => setSecuritySettings((current) => current
                ? {
                    ...current,
                    antivirus_mode: event.target.value,
                }
                : current)}>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <FormControl size="small" fullWidth>
                  ...
                  <Select labelId="ops-pii-mode" label="PII Redaction Mode" value={securitySettings.pii_redaction_mode} onChange={(event) => setSecuritySettings((current) => current
                ? {
                    ...current,
                    pii_redaction_mode: event.target.value,
                }
                : current)}>
                    <MenuItem value="off">off</MenuItem>
      ...
      {(canManageLimits || canViewHealth) && (<Grid container spacing={2}>
          {canManageLimits && (<Grid size={{ xs: 12, lg: 5 }}>
              <Paper sx={{ p: 2, height: '100%' }}>
                <Stack spacing={1.5}>
                  <Typography variant="h6">Usage Limits</Typography>

                  <TextField size="small" label="Storage quota (MB)" type="number" value={limits?.storage_quota_mb ?? ''} onChange={(event) => setLimits((current) => current
                    ? {
                        ...current,
                  ...
                  <TextField size="small" label="Max exports/day" type="number" value={limits?.max_exports_per_day ?? ''} onChange={(event) => setLimits((current) => current
                    ? {
                        ...current,
                  ...
                  <TextField size="small" label="Max users" type="number" value={limits?.max_users ?? ''} onChange={(event) => setLimits((current) => current
                    ? {
                        ...current,
          ...
          {canViewHealth && (<Grid size={{ xs: 12, lg: canManageLimits ? 7 : 12 }}>
              <Paper sx={{ p: 2, height: '100%' }}>
                <Stack spacing={2}>
                  <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={1}>
                    ...
                    <FormControl size="small" sx={{ minWidth: 180 }}>
                      ...
                      <Select labelId="ops-window-hours" label="Window" value={windowHours} onChange={(event) => setWindowHours(Number(event.target.value))}>
                  ...
                  <Grid container spacing={1.5}>
                    ...
                    <Grid size={{ xs: 12, md: 3 }}>
                      <Card>
                        <CardContent>
                          <Typography variant="body2" color="text.secondary">Request Error Rate</Typography>
                          <Typography variant="h5">{health?.request_error_rate ?? 0}%</Typography>
                          <Typography variant="caption">API diagnostics window</Typography>
                        </CardContent>
                      </Card>
                    </Grid>
                    <Grid size={{ xs: 12, md: 3 }}>
                      <Card>
                        <CardContent>
                          <Typography variant="body2" color="text.secondary">P95 API Latency</Typography>
                          <Typography variant="h5">{health?.p95_api_latency_ms ?? '-'}</Typography>
                          <Typography variant="caption">ms (sampled API requests)</Typography>
                        </CardContent>
                      </Card>
                    </Grid>
                    <Grid size={{ xs: 12, md: 3 }}>
                      <Card>
                        <CardContent>
                          <Typography variant="body2" color="text.secondary">Sync Retry Rate</Typography>
                          <Typography variant="h5">{health?.sync_retry_rate ?? 0}%</Typography>
                          <Typography variant="caption">retried operation share</Typography>
                        </CardContent>
                      </Card>
                    </Grid>
                    <Grid size={{ xs: 12, md: 3 }}>
                      <Card>
                        <CardContent>
                          <Typography variant="body2" color="text.secondary">Websocket Failures (24h)</Typography>
                          <Typography variant="h5">{health?.websocket_delivery_failures_last_24h ?? 0}</Typography>
                          <Typography variant="caption">delivery failure events</Typography>
                        </CardContent>
                      </Card>
                    </Grid>
                  </Grid>
                  ...
                  <Grid container spacing={1.5}>
                    <Grid size={{ xs: 12, md: 5 }}>
                      <Paper variant="outlined" sx={{ p: 1.5 }}>
                        ...
                        <Stack spacing={0.7}>
                          {syncBreakdownRows.map((row) => (<Box key={row.status} display="flex" justifyContent="space-between">
                    ...
                    <Grid size={{ xs: 12, md: 7 }}>
                      <Paper variant="outlined" sx={{ p: 1.5 }}>
                        ...
                        <Stack spacing={1}>
                          {(health?.storage_failures.recent ?? []).map((event, index) => (<Box key={`${event.source}-${event.occurred_at ?? index}`}>
                          ...
                          {(health?.storage_failures.recent ?? []).length === 0 && (<Typography variant="body2" color="text.secondary">No storage failures recorded in this window.</Typography>)}
                        </Stack>
                      </Paper>
                    </Grid>
                  </Grid>
      ...
      {canManageSupport && (<Grid container spacing={2}>
            ...
            <Paper sx={{ p: 2, height: '100%' }}>
              <Stack spacing={1.5}>
                ...
                <FormControl size="small" fullWidth>
                  ...
                  <Select labelId="ops-target-user" label="Target user" value={selectedUserId} onChange={(event) => setSelectedUserId(String(event.target.value))}>
                ...
                <TextField size="small" label="Tour key for reset (optional)" value={resetTourKey} onChange={(event) => setResetTourKey(event.target.value)}/>

                <TextField size="small" label="Replay tours (csv keys)" value={replayTourKeys} onChange={(event) => setReplayTourKeys(event.target.value)}/>
          ...
          <Grid size={{ xs: 12, lg: 6 }}>
            <Paper sx={{ p: 2, height: '100%' }}>
              <Stack spacing={1.5}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                  <TextField size="small" label="Invite email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} fullWidth/>
                  <TextField size="small" label="Expires in days" type="number" value={inviteExpiryDays} onChange={(event) => setInviteExpiryDays(event.target.value)} sx={{ width: { xs: '100%', sm: 150 } }}/>
                ...
                <Table size="small">
                  ...
                  <TableBody>
                    {(invites?.data ?? []).map((invite) => (<TableRow key={invite.id}>
                        <TableCell>{invite.email}</TableCell>
                        <TableCell>{invite.status}</TableCell>
                        <TableCell>{invite.last_sent_at ? new Date(invite.last_sent_at).toLocaleString() : 'n/a'}</TableCell>
                        <TableCell align="right">
                          <Button size="small" onClick={() => void resendInvite(invite.id)} disabled={invite.status !== 'pending'}>
                            Resend
                          </Button>
                        </TableCell>
                      </TableRow>))}
