...
export const AccessControlPage = () => {
  ...
  const [members, setMembers] = useState<ProjectRoleMember[]>([])
  const [membersForDelegation, setMembersForDelegation] = useState<UserSummary[]>([])
  const [roleDrafts, setRoleDrafts] = useState<Record<number, string[]>>({})
  const [savingUserId, setSavingUserId] = useState<number | null>(null)

  const [presets, setPresets] = useState<PermissionPresetRecord[]>([])
  const [presetKey, setPresetKey] = useState<string>('')
  const [diffUserId, setDiffUserId] = useState<number | null>(null)
  const [diff, setDiff] = useState<PermissionDiffResponse | null>(null)
  ...
  const [newTeamCompanyId, setNewTeamCompanyId] = useState<string>('')

  const [delegations, setDelegations] = useState<DelegationRuleRecord[]>([])
  const [delegatorId, setDelegatorId] = useState<string>('')
  const [delegateId, setDelegateId] = useState<string>('')
  const [delegationScope, setDelegationScope] = useState<'all' | 'assignments' | 'approvals'>('assignments')
  ...
  const loadProjectRoles = async (selectedProjectId: number) => {
    const response = await api.get<{ data: RbacProjectRolesResponse }>(`/api/projects/${selectedProjectId}/roles`)
    const projectMembers = response.data.data.members
    setMembers(projectMembers)
    setRoleDrafts(
      projectMembers.reduce<Record<number, string[]>>((acc, member) => {
  ...
  const loadOrganizationMembers = async (selectedProjectId: number) => {
  ...
  const loadPresets = async () => {
    const response = await api.get<{ data: PermissionPresetRecord[] }>('/api/rbac/permission-presets')
  ...
  const loadStakeholders = async (selectedProjectId: number) => {
  ...
  const loadDelegations = async (selectedProjectId: number) => {
    const response = await api.get<{ data: DelegationRuleRecord[] }>('/api/delegations', {
      params: {
        project_id: selectedProjectId,
        active_only: false,
      },
  ...
  const saveRoleOverride = async (member: ProjectRoleMember) => {
    if (!projectId) {
      return
    }
  ...
  const comparePreset = async () => {
    ...
    try {
      const response = await api.get<{ data: PermissionDiffResponse }>('/api/rbac/permission-diff', {
        params: {
  ...
  const deleteDelegation = async (delegationId: number) => {
    if (!projectId) {
      return
    }
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ xs: 'stretch', md: 'center' }}>
          <Box flexGrow={1}>
            ...
            <Typography color="text.secondary">
              Project-level role overrides, stakeholder assignment groups, delegation windows, and permission preset diffing.
            </Typography>
          </Box>
                  ...
                  <TableCell>
                    <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
                      {member.org_roles.map((role) => (
                        <Chip key={`${member.id}-org-${role}`} label={role} size="small" variant="outlined" />
                      ))}
                    </Stack>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
                      {member.effective_roles.map((role) => (
                        <Chip key={`${member.id}-effective-${role}`} label={role} size="small" color="primary" variant="outlined" />
                      ))}
                    </Stack>
                  </TableCell>
          ...
          <Paper sx={{ p: 2, height: '100%' }}>
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={1} mb={1.5}>
              ...
              <FormControl size="small" sx={{ minWidth: 160 }}>
                ...
                <Select
                  ...
                  onChange={(event) => setNewCompanyType(String(event.target.value))}
                >
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={1} mb={2}>
              ...
              <FormControl size="small" sx={{ minWidth: 200 }}>
                ...
                <Select
                  labelId="team-company"
                  label="Company"
                  value={newTeamCompanyId}
                  onChange={(event) => setNewTeamCompanyId(String(event.target.value))}
                >
            ...
            <Stack spacing={1.5}>
              <FormControl size="small" fullWidth>
                ...
                <Select
                  ...
                  label="Preset"
                  value={presetKey}
                  onChange={(event) => setPresetKey(String(event.target.value))}
                >
                  {presets.map((preset) => (
              ...
              <FormControl size="small" fullWidth>
                ...
                <Select
                  labelId="diff-user"
                  label="Target User"
                  value={diffUserId ? String(diffUserId) : ''}
                  onChange={(event) => setDiffUserId(event.target.value ? Number(event.target.value) : null)}
                >
            ...
            <Stack spacing={1.2} mb={2}>
              <FormControl size="small" fullWidth>
                ...
                <Select
                  ...
                  label="Delegator"
                  value={delegatorId}
                  onChange={(event) => setDelegatorId(String(event.target.value))}
                >
              ...
              <FormControl size="small" fullWidth>
                ...
                <Select
                  ...
                  label="Delegate"
                  value={delegateId}
                  onChange={(event) => setDelegateId(String(event.target.value))}
                >
                  {membersForDelegation.map((member) => (
              ...
              <FormControl size="small" fullWidth>
                ...
                <Select
                  labelId="delegation-scope"
                  label="Scope"
                  value={delegationScope}
                  onChange={(event) => setDelegationScope(event.target.value as 'all' | 'assignments' | 'approvals')}
                >
