import { Alert, Button, Chip, FormControl, Grid, InputLabel, MenuItem, Paper, Select, Stack, } from '@mui/material';
...
export const ExportCenterPage = () => {
    ...
    const exportTourSteps = useMemo(() => [
        {
            ...
            attachTo: { element: '#exports-create', on: 'bottom' },
        },
        {
            id: 'exports_center',
            title: 'Export Center',
            text: 'Track queued and completed exports, then download when ready.',
            attachTo: { element: '#exports-grid', on: 'top' },
        },
    ], []);
    useFeatureTour({
        tourKey: 'exports',
    ...
    const queuedCount = useMemo(() => jobs.filter((job) => job.status === 'queued' || job.status === 'processing').length, [jobs]);
    ...
    return (<Stack spacing={2}>
      ...
      <PageHero title="Export Center" description="Queue PDF, CSV, and XLSX exports and download files as soon as processing completes." badges={<Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
        ...
        <Grid container spacing={2} mt={0.2} alignItems="center">
          <Grid size={{ xs: 12, md: 3 }}>
            <FormControl fullWidth size="small">
              ...
              <Select labelId="export-type-label" label="Type" value={type} onChange={(event) => setType(event.target.value)}>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <FormControl fullWidth size="small">
              ...
              <Select labelId="export-project-label" label="Project" value={selectedProjectId} onChange={(event) => setSelectedProjectId(event.target.value ? Number(event.target.value) : '')}>
      ...
      <Paper sx={{ p: 1.5 }} id="exports-grid">
        <DataGrid autoHeight rows={jobs} columns={columns} loading={loading} disableRowSelectionOnClick pageSizeOptions={[10, 20, 50]} sx={{ border: 0 }}/>
      </Paper>
    </Stack>);
};
