import { Alert, Box, Button, Chip, Divider, List, ListItem, ListItemText, Paper, Stack, TextField, Typography, } from '@mui/material';
...
export const InspectionSubmissionDetailPage = () => {
    ...
    return (<Stack spacing={2}>
      ...
      {submission && (<>
          <PageHero title={submission.reference} description={`${submission.template?.name ?? 'Template'} | ${submission.project?.code ?? 'No project'}`} actions={<Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                <Chip color={statusColorMap[submission.status] ?? 'default'} label={submission.status} sx={{ bgcolor: 'rgba(255,255,255,0.22)', color: '#FFFFFF' }}/>
                <Button variant="outlined" onClick={() => navigate('/inspections/submissions')} sx={{ borderColor: 'rgba(255,255,255,0.45)', color: '#FFFFFF' }}>
                  Back
                </Button>
                {canUpdate && (<Button variant="outlined" onClick={() => void saveForm()} disabled={busy} sx={{ borderColor: 'rgba(255,255,255,0.45)', color: '#FFFFFF' }}>
                    Save Draft
                  </Button>)}
                {canSubmit && submission.status === 'draft' && (<Button variant="contained" onClick={() => void submitForReview()} disabled={busy} sx={{ bgcolor: 'rgba(255,255,255,0.22)', color: '#FFFFFF' }}>
                    Submit
                  </Button>)}
              </Stack>}/>

          <Box display="grid" gridTemplateColumns={{ xs: '1fr', sm: '1fr 1fr', lg: 'repeat(4, 1fr)' }} gap={1.2}>
            <StatCard label="Approval Steps" value={metrics.steps} tone="primary"/>
            <StatCard label="Approved Steps" value={metrics.approved} tone="success"/>
            <StatCard label="Signatures" value={metrics.signatures} tone="secondary"/>
            <StatCard label="Linked Requests" value={metrics.requests} tone="neutral"/>
          </Box>

          <Box display="grid" gridTemplateColumns={{ xs: '1fr', lg: '1.4fr 0.8fr' }} gap={2}>
            <DynamicInspectionForm template={submission.template} value={formData} onChange={canUpdate ? setFormData : undefined} disabled={!canUpdate || !['draft', 'in_review'].includes(submission.status)}/>
            ...
            <Stack spacing={1.5}>
              <Paper sx={{ p: 2 }} id="approval-timeline">
                ...
                <List dense>
                  {(submission.approvals ?? []).map((approval) => (<ListItem key={approval.id} sx={{ px: 0 }}>
                      <ListItemText primary={`#${approval.step_order} ${approval.step_name ?? approval.role_name}`} secondary={`${approval.role_name} | ${approval.status}${approval.acted_at ? ` | ${new Date(approval.acted_at).toLocaleString()}` : ''}`}/>
                      <Chip size="small" label={approval.status} color={approval.status === 'approved'
                    ? 'success'
              ...
              <Paper sx={{ p: 2 }} id="approval-actions">
                ...
                : 'No pending step'}
                  ...
                  <TextField size="small" label="Decision Notes" value={decisionNotes} onChange={(event) => setDecisionNotes(event.target.value)} multiline minRows={2} disabled={!canReview || !pendingApproval || busy}/>

                  <Stack direction="row" spacing={1}>
                    <Button variant="contained" color="success" disabled={!canReview || !pendingApproval || busy} onClick={() => void decide('approve')}>
                      Approve
                    </Button>
                    <Button variant="contained" color="error" disabled={!canReview || !pendingApproval || busy} onClick={() => void decide('reject')}>
                      Reject
                    </Button>
                  </Stack>
                </Stack>
              ...
              <Paper sx={{ p: 2 }}>
                <Stack spacing={1.2}>
                  ...
                  <TextField size="small" label="Message" value={approvalMessageBody} onChange={(event) => setApprovalMessageBody(event.target.value)} multiline minRows={2} disabled={busy}/>
                  <Button variant="outlined" disabled={!approvalMessageBody.trim() || busy} onClick={() => void postApprovalMessage()}>
                    Post Message
                  </Button>

                  <List dense>
                    {approvalMessages.length === 0 ? (<ListItem sx={{ px: 0 }}>
                        <ListItemText primary="No chat messages yet."/>
                      </ListItem>) : (approvalMessages.map((message) => (<ListItem key={message.id} sx={{ px: 0, alignItems: 'flex-start' }}>
                          <ListItemText primary={`${message.user?.name ?? 'System'} | ${message.message_type}`} secondary={`${message.body}${message.approval?.step_order ? ` | Step ${message.approval.step_order}` : ''} | ${new Date(message.created_at).toLocaleString()}`}/>
                        </ListItem>)))}
                  </List>
                </Stack>
              </Paper>
              ...
              <Paper sx={{ p: 2 }} id="signature-panel">
                ...
                : 'No signature required for current step.'}
                  ...
                  <Button variant="contained" disabled={!canSign || !pendingApproval?.requires_signature || !signatureData || busy} onClick={() => void uploadSignature()}>
                    Upload Signature
                  </Button>
                  ...
                  <List dense>
                    {(submission.signatures ?? []).map((signature) => (<ListItem key={signature.id} sx={{ px: 0 }}>
                        <ListItemText primary={`${signature.signer?.name ?? 'User'} | ${new Date(signature.signed_at).toLocaleString()}`} secondary={signature.context}/>
                        <Button size="small" onClick={() => window.open(`/api/inspections/signatures/${signature.id}/download`, '_blank')}>
                          Download
                        </Button>
                      </ListItem>))}
                  </List>
              ...
              <Paper sx={{ p: 2 }}>
                ...
                <List dense>
                  {(submission.requests ?? []).map((request) => (<ListItem key={request.id} sx={{ px: 0 }}>
                      <ListItemText primary={`${request.reference} | ${request.request_type.toUpperCase()} | ${request.status}`} secondary={request.title}/>
                    </ListItem>))}
                </List>
