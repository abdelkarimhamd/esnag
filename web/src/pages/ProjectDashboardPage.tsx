...
import type { Drawing, ProjectSummary, SnagStatus } from '../types'
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [projectResponse, dashboardResponse] = await Promise.all([
        api.get<{ data: ProjectDetail }>(`/api/projects/${projectId}`),
        api.get<{ data: DashboardPayload }>(`/api/projects/${projectId}/dashboard`),
      ])
    ...
    <Stack spacing={3}>
      ...
      {project && (
        <PageHero
          ...
          actions={
            <Button component={RouterLink} to="/projects" variant="outlined" sx={{ borderColor: 'rgba(255,255,255,0.42)', color: '#FFFFFF' }}>
              Back to Projects
            </Button>
          }
          badges={
      ...
      {project?.is_training && project.training_locked && (
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            <Paper sx={{ p: 1.8, borderRadius: 3 }}>
              ...
              <Stack direction="row" spacing={0.8} mt={1} flexWrap="wrap" useFlexGap>
                {Object.entries(dashboard.summary.status_breakdown).map(([status, total]) => (
                  <Chip key={status} size="small" color={snagStatusChipColor(status as SnagStatus)} label={`${formatStatusLabel(status)} ${total}`} />
                ))}
              </Stack>
            </Paper>
          </Grid>
