...
export const NotificationsMenu = ({ canView }) => {
    ...
    useEffect(() => {
        if (!canView || !activeOrganizationId) {
            return;
        }
    ...
    }, [activeOrganizationId, canView, loadNotifications]);
    ...
    return (<>
      ...
      <Menu open={open} anchorEl={anchorEl} onClose={closeMenu} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} transformOrigin={{ vertical: 'top', horizontal: 'right' }} PaperProps={{ sx: { width: 360, p: 1 } }}>
        ...
        <List sx={{ maxHeight: 380, overflowY: 'auto' }}>
          {notifications.length === 0 ? (<ListItem>
            ...
            </ListItem>) : (notifications.map((notification) => {
            const text = notificationText(notification);
            return (<ListItem key={notification.id} sx={{
