import { Accordion, AccordionDetails, AccordionSummary, Box, Button, FormControl, IconButton, MenuItem, Select, Stack, Switch, TextField, Typography } from '@mui/material';
...
const DISCIPLINES = [
  'Civil', 'Structural', 'Architectural', 'Finishing', 'Mechanical (HVAC)', 'Electrical',
  'Plumbing', 'Fire Alarm', 'Fire Fighting', 'ELV / CCTV', 'ELV / Access Control',
  'Landscaping / Irrigation', 'Safety', 'Other',
];
...
// The snag creation form, extracted from the old modal so it can be hosted on a
// full page (CreateSnagPage). Owns all field state, DLP/TOC handling and photo
// staging; the host supplies the drawing/pin context and an async onSubmit that
// performs the POST + photo upload.
export const SnagForm = ({
  drawing, locationOptions = [], rootCauseCategories = [], members = [], companies = [], teams = [],
  canAssign = false, canManageToc = false, pin, suggestedLocation, onSubmit, onCancel, submitLabel = 'Create snag',
}) => {
  ...
  // Load the project's active certificates when DLP is switched on, so a DLP
  // defect can be bound to a real Taking-Over Certificate.
  useEffect(() => {
    if (!isDlp || !drawing?.project_id) {
      return;
    }
    let cancelled = false;
    void api
  ...
  }, [isDlp, drawing?.project_id]);

  // Seed the location from the async pin suggestion without clobbering user input.
  useEffect(() => {
  ...
  // Configurable categories + Area master data for the project.
  useEffect(() => {
  ...
  const pinLabel = pin ? `${Math.round(pin.x * 100)}%, ${Math.round(pin.y * 100)}%` : 'N/A';
  const zoneLine = suggestedLocationEntry
    ? `${suggestedLocationEntry.code ?? 'LOC'} · ${suggestedLocationEntry.name}`
    : (pin ? `Pin at ${pinLabel}` : 'Awaiting pin');
  ...
  return (
    <Box>
      ...
      {/* DLP toggle */}
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '10px', mb: '15px', p: '11px 12px', borderRadius: '10px', border: `1px solid ${isDlp ? 'rgba(192,138,35,0.4)' : BRAND.borderStrong}`, background: isDlp ? 'rgba(192,138,35,0.08)' : BRAND.panel }}>
      ...
      <Box sx={{ mb: '15px' }}>
        ...
        <Box sx={{ display: 'flex', gap: '6px' }}>
          {PRIORITY_ORDER.map((value) => {
            ...
            return (
              <Box key={value} role="button" tabIndex={0} aria-pressed={selected} onClick={() => setPriority(value)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setPriority(value); } }} sx={{ flex: 1, textAlign: 'center', p: '8px 0', borderRadius: '8px', fontSize: 12, cursor: 'pointer', userSelect: 'none', fontWeight: selected ? 700 : 600, background: selected ? (critical ? BRAND.red : 'rgba(36,72,143,0.10)') : '#F1F3F8', color: selected ? (critical ? '#fff' : BRAND.navy) : BRAND.muted }}>
      ...
      <Box sx={{ mb: '15px' }}>
        ...
        <Box sx={{ display: 'flex', gap: '6px' }}>
          {SEVERITY_ORDER.map((value) => {
            ...
            return (
              <Box key={value} role="button" tabIndex={0} aria-pressed={selected} onClick={() => setSeverity(value)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSeverity(value); } }} sx={{ flex: 1, textAlign: 'center', p: '8px 0', borderRadius: '8px', fontSize: 12, cursor: 'pointer', userSelect: 'none', fontWeight: selected ? 700 : 600, background: selected ? (major ? BRAND.red : 'rgba(36,72,143,0.10)') : '#F1F3F8', color: selected ? (major ? '#fff' : BRAND.navy) : BRAND.muted }}>
      ...
      <Box sx={{ mb: '15px', display: 'flex', gap: '10px' }}>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <FormControl fullWidth>
            <Select displayEmpty value={areaId} onChange={(event) => { setAreaId(String(event.target.value)); setBuildingId(''); }} sx={selectSx}>
      ...
      <Box sx={{ mb: '15px' }}>
        ...
        <TextField value={locationText} onChange={(event) => setLocationText(event.target.value)} fullWidth placeholder="Or type a specific location…" InputProps={{ sx: { ...inputSx, color: BRAND.ink } }} sx={{ mt: '8px' }} />
      </Box>
      ...
      <Box sx={{ mt: '14px' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
          <Box component="label" role="button" aria-label="Add photo" sx={{ width: 56, height: 56, borderRadius: '10px', border: `1.5px dashed ${isDlp && photos.length === 0 ? BRAND.amber : 'rgba(20,38,66,0.2)'}`, color: isDlp && photos.length === 0 ? BRAND.amber : '#A6B0BF', display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none', cursor: 'pointer' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
              <line x1="12" y1="5" x2="12" y2="19" />
              <line x1="5" y1="12" x2="19" y2="12" />
            </svg>
            <input type="file" accept="image/*,.pdf" multiple hidden onChange={(event) => { addPhotos(event.target.files); event.target.value = ''; }} />
          </Box>
          {photos.map((file, index) => {
            ...
            return (
              <Box key={url ?? index} sx={{ position: 'relative', width: 56, height: 56, flex: 'none' }}>
                {isPdf ? (
                  <Box sx={{ width: 56, height: 56, borderRadius: '10px', border: `1px solid ${BRAND.borderStrong}`, background: '#F1F3F8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: FONT_MONO, fontSize: 10, fontWeight: 600, color: BRAND.inkSoft }}>PDF</Box>
                ) : (
                ...
                <IconButton aria-label="Remove photo" onClick={() => removePhoto(index)} sx={{ position: 'absolute', top: -7, right: -7, width: 18, height: 18, p: 0, bgcolor: BRAND.ink, color: '#fff', '&:hover': { bgcolor: BRAND.red } }}>
      ...
      {/* Footer actions */}
      <Box sx={{ display: 'flex', gap: '10px', mt: '20px', pt: '16px', borderTop: `1px solid ${BRAND.border}` }}>
        <Button onClick={onCancel} sx={{ p: '10px 16px', borderRadius: '10px', border: `1px solid ${BRAND.borderStrong}`, fontSize: 13, fontWeight: 600, color: BRAND.inkSoft, bgcolor: BRAND.panel, boxShadow: 'none', '&:hover': { bgcolor: '#F1F3F8', boxShadow: 'none' } }}>
          Cancel
        </Button>
